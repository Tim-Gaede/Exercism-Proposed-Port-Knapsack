...
the *iᵗʰ item and `vi` is the value for that item.

For example:

Items: [("weight" => 5, "value" => 10 ), ("weight" => 4, "value" => 40 ),
        ("weight" => 6, "value" => 30 ), ("weight" => 4, "value" => 50 )]
...
Note that, when trying to submit an exercise, make sure the solution is in the `$EXERCISM_WORKSPACE/julia/knapsack` directory.
